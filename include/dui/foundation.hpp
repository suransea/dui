...
namespace detail {
template<class T>
inline constexpr unsigned char type_token_storage{};
}

template<class T>
[[nodiscard]] constexpr TypeToken type_token() {
...
template<class T>
[[nodiscard]] Key make_key(T&& value) {
...
struct MemberKey {
    template<class T>
    [[nodiscard]] decltype(auto) operator()(const T& value) const {
        return value.*Member;
    }
};

template<auto Member>
inline constexpr MemberKey<Member> key{};
