...
private:
    ...
    struct EnvironmentSlot final : DependencySource {
        ...
        explicit EnvironmentSlot(T initial) :
            type(type_token<T>()),
            value(std::make_any<T>(std::move(initial))) {}
    ...
    template<class T>
    struct ResourceHolder final : ResourceValue {
        template<class U>
        explicit ResourceHolder(U&& initial) : value(std::forward<U>(initial)) {}

        void request_cancel() noexcept override {
            if constexpr (requires(T& resource) { { resource.request_stop() } noexcept; }) {
                value.request_stop();
            }
        }
...
struct ElementAccess {
    ...
    static std::string& debug_value(Element& element) {
        return element.debug_value_;
    }

    static std::any& descriptor(Element& element) {
        return element.descriptor_;
    }

    static bool dirty(const Element& element) {
        return element.dirty_;
    }

    static bool rebuilding_dirty(const Element& element) {
        return element.rebuilding_dirty_;
    }

    static void set_rebuild(
        Element& element,
        void (*rebuild)(Element&, BuildOwner&)
    ) {
        element.rebuild_ = rebuild;
    }

    static void rebuild(Element& element, BuildOwner& owner) {
        element.rebuild_(element, owner);
    }
    ...
    static bool has_focus_node(const Element& element) {
        return element.focus_node_ != nullptr;
    }

    static std::shared_ptr<FocusNode> ensure_focus_node(
        Element& element,
        BuildOwner& owner,
        ...
        if (element.focus_node_ == nullptr) {
            ...
            element.focus_node_ = owner.focus_manager_.create_node(
                std::move(parent),
                std::move(handler),
                can_focus
            );
    ...
    template<class T>
    static void set_environment(Element& element, T value) {
    ...
    static Render& ensure_render_object(
        Element& element,
        BuildOwner& owner,
        Arguments&&... arguments
    ) {
        if (element.render_object_ == nullptr) {
            element.render_object_ = std::make_unique<Render>(
                owner.render_owner_,
...
public:
    ...
    template<class T>
    [[nodiscard]] const T& watch(Signal<T>& signal);

    template<class T>
    [[nodiscard]] const T& environment();

    template<fixed_string Name, class Factory>
    [[nodiscard]] auto& resource(Factory&& factory);
...
public:
    ...
    ~BuildScope() {
        detail::active_build_context = previous_;
    }
...
template<class T>
class Signal final : public DependencySource {
public:
...
private:
    ...
    StateHandle(
        std::weak_ptr<OwnerLifetime> lifetime,
        Element::Id element,
        std::uint64_t generation,
        std::uint64_t slot
...
StateHandle<std::decay_t<T>> BuildContext::state(T&& initial) {
    using Value = std::decay_t<T>;
    static_assert(std::copy_constructible<Value>, "DUI state values must currently be copy constructible");
    ...
    if (iterator == element_->state_.end()) {
        ...
        iterator = element_->state_.emplace(
            slot,
            Element::StateSlot{
    ...
    return StateHandle<Value>{
        owner_->lifetime_, element_->id_, element_->generation_, slot
    };
}

template<class T>
const T& BuildContext::watch(Signal<T>& signal) {
    observe(signal);
    return signal.get();
}

template<class T>
const T& BuildContext::environment() {
...
template<fixed_string Name, class Factory>
auto& BuildContext::resource(Factory&& factory) {
    ...
    if (found == element_->resources_.end()) {
        ...
        found = element_->resources_.emplace(
            slot,
            Element::ResourceSlot{
                std::string(Name.view()),
                type_token<Value>(),
                std::move(holder)
            }
...
template<fixed_string Name>
CancellationToken BuildContext::cancellation() {
