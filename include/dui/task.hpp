...
public:
  ...
  public:
    ...
    template <class Promise> void await_suspend(std::coroutine_handle<Promise> coroutine) const {
...
public:
  struct promise_type {
    ...
    [[nodiscard]] std::weak_ptr<detail::CoroutineControl> control_token() const { return control; }
  ...
  [[nodiscard]] bool started() const { return control_ != nullptr && control_->started; }

  [[nodiscard]] bool done() const { return control_ != nullptr && control_->coroutine.done(); }
...
private:
  explicit Task(std::shared_ptr<detail::CoroutineControl> control) : control_(std::move(control)) {}
  ...
  [[nodiscard]] static std::coroutine_handle<promise_type>
  typed_handle(const detail::CoroutineControl& control) {
