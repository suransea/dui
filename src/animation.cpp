...
std::size_t TickerScheduler::live_ticker_count() const {
  return static_cast<std::size_t>(
    std::ranges::count_if(tickers_, [](const auto& ticker) { return !ticker.expired(); }));
}

AnimationController::AnimationController(TickerScheduler& scheduler, FrameDuration duration,
                                         ValueCallback callback)
  : duration_(duration), callback_(std::move(callback)),
    ticker_(scheduler.create_ticker([this](FrameDuration elapsed) { on_tick(elapsed); })) {
...
void AnimationController::stop() { ticker_->stop(); }
...
void AnimationController::set_value(double value) {
  ...
  value_ = value;
  elapsed_ = duration_ * value_;
  status_ = value_ <= 0.0   ? AnimationStatus::dismissed
            : value_ >= 1.0 ? AnimationStatus::completed
                            : status_;
  publish();
}
