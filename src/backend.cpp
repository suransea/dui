...
bool SurfaceRequest::valid() const {
    return std::isfinite(logical_size.width) && std::isfinite(logical_size.height)
        ...
        && ((logical_size.width == 0.0 || logical_size.height == 0.0)
            ...
            : physical_width != 0 && physical_height != 0)
        && std::abs(static_cast<double>(physical_width) - logical_size.width * device_pixel_ratio) <= 1.0
        && std::abs(static_cast<double>(physical_height) - logical_size.height * device_pixel_ratio) <= 1.0;
}
...
SurfaceFrame::SurfaceFrame(
    ...
    if (delegate_ == nullptr) {
        throw std::invalid_argument("Ready surface frame requires dimensions, generation, and delegate");
    }
...
SurfaceAcquisition SurfaceAcquisition::failed(SurfaceAcquireStatus status) {
    if (status != SurfaceAcquireStatus::unavailable
        && status != SurfaceAcquireStatus::out_of_date
...
SurfaceAcquisition RasterSurface::acquire(const SurfaceRequest& request) {
    ...
    if (descriptor.width != request.physical_width
        || descriptor.height != request.physical_height
...
public:
    ...
    void wait_idle() {
        ...
        state_->idle.wait(lock, [&] {
            return state_->pending.empty() && !state_->rendering;
        });
        rethrow_failure();
    }
...
void RasterThread::request_stop() {
    impl_->request_stop();
}

void RasterThread::wait_idle() {
    impl_->wait_idle();
}

std::size_t RasterThread::pending_count() const {
