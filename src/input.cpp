...
void GestureArena::accept(PointerId pointer, const GestureArenaMember& member) {
    ...
    const auto candidate = std::ranges::find_if(found->second.members, [&](const auto& value) {
...
void GestureArena::reject(PointerId pointer, const GestureArenaMember& member) {
    ...
    const auto candidate = std::ranges::find_if(arena.members, [&](const auto& value) {
        return value.get() == &member;
    });
...
bool GestureArena::contains(PointerId pointer) const {
    return storage_->arenas.contains(pointer);
}

void GestureArena::resolve_winner(
    PointerId pointer,
    std::shared_ptr<GestureArenaMember> winner
) {
    const auto storage = storage_;
...
void FocusNode::request_focus() {
    manager().request_focus(*this);
}
...
std::shared_ptr<FocusNode> FocusManager::create_node(
    ...
    auto node = std::shared_ptr<FocusNode>{new FocusNode{
        lifetime_,
        id,
        parent_id,
        can_focus,
...
bool TextEditingValue::valid() const {
    return selection.valid_for(text)
        && (!composing.has_value() || composing->valid_for(text));
}
