...
public:
    ...
    ~Impl() {
        ...
        const bool was_composing = state->active.has_value()
            && state->active->value.composing.has_value();
        state->active.reset();
        ++state->generation;
        state_->alive = false;
        if (was_composing) {
    ...
    [[nodiscard]] TextInputSessionId start(
...
private:
    ...
    [[nodiscard]] static Session* find(State& state, TextInputSessionId session) {
        return state.active.has_value() && state.active->id == session
            ? &*state.active
            : nullptr;
    }
    ...
    static void apply_rect(const State& state, Rect rect) {
        ...
        const LONG y = native_coordinate(
    ...
    [[nodiscard]] static std::wstring read_composition(HIMC context, DWORD index) {
        ...
        if (bytes != 0
            && ImmGetCompositionStringW(context, index, result.data(), static_cast<DWORD>(bytes))
                != bytes) {
    ...
    [[nodiscard]] static TextEditingValue replace_range(
        TextEditingValue value,
        TextRange range,
    ...
    [[nodiscard]] static bool emit_value(
    ...
    static void process_character(const std::shared_ptr<State>& state, wchar_t character) {
        ...
        if (character == L'\r') {
            if (!session.configuration.multiline || session.configuration.action != TextInputAction::none) {
        ...
        if (character == L'\b') {
            ...
            static_cast<void>(emit_value(
                state,
                replace_range(std::move(value), range, {}, 0, false)
            ));
            return;
        }
        ...
        static_cast<void>(emit_value(
            state,
            replace_range(session.value, session.value.selection, inserted, inserted.size(), false)
        ));
    }
    ...
    static void process_composition(const std::shared_ptr<State>& state, LPARAM flags) {
        ...
        std::optional<std::pair<std::string, std::size_t>> composition;
        {
            ...
            if ((flags & GCS_COMPSTR) != 0) {
                ...
                if (cursor != 0
                    && static_cast<std::size_t>(cursor) < wide.size()
        ...
        if (result.has_value()) {
            const TextRange range = state->active->value.composing.value_or(
                state->active->value.selection
            );
            state->active->before_composition.reset();
            if (!emit_value(
                state,
...
TextInputSessionId TextInputBackend::start_text_input(
    std::weak_ptr<TextInputClient> client,
    TextInputConfiguration configuration,
    TextEditingValue initial_value
) {
...
void TextInputBackend::stop_text_input(TextInputSessionId session) {
    impl_->stop(session);
}
...
std::optional<std::intptr_t> TextInputBackend::handle_message(
    std::uint32_t message,
    std::uintptr_t wparam,
    std::intptr_t lparam
) noexcept {
