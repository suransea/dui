...
DependencySource::~DependencySource() {
    for (const Subscriber& subscriber : subscribers_) {
        subscriber.owner->forget_dependency(
            subscriber.id,
...
BuildOwner::BuildOwner() : lifetime_(std::make_shared<OwnerLifetime>()) {
...
std::unique_ptr<Element> BuildOwner::make_element(
    TypeToken type,
    Key key,
    ...
    auto element = std::unique_ptr<Element>{new Element{
        id,
        id,
        parent == nullptr ? 0 : parent->depth_ + 1,
        type,
        std::move(key),
        std::move(debug_name)
    }};
...
void BuildOwner::forget_dependency(
    Element::Id id,
...
DisplayList BuildOwner::frame(BoxConstraints viewport) {
