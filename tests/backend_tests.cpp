...
void surface_frame_is_exactly_once_transaction() {
    ...
    {
        dui::SurfaceFrame frame{
            {2, 2, 8, dui::SurfacePixelFormat::rgba8888_srgb_premultiplied, 1},
    ...
    {
        dui::SurfaceFrame frame{
            {1, 1, 4, dui::SurfacePixelFormat::rgba8888_srgb_premultiplied, 2},
            std::make_unique<MemoryFrameDelegate>(abandoned, 4)
        };
    }
    require(abandoned->presents == 0 && abandoned->abandons == 1, "dropped frame was not abandoned once");
}
...
void surface_contract_rejects_invalid_frames_and_acquisitions() {
    ...
    try {
        dui::SurfaceFrame frame{
            {2, 2, 7, dui::SurfacePixelFormat::rgba8888_srgb_premultiplied, 1},
    ...
    try {
        static_cast<void>(dui::SurfaceAcquisition::failed(
            static_cast<dui::SurfaceAcquireStatus>(99)
        ));
    } catch (const std::invalid_argument&) {
        invalid_enum = true;
    }
    ...
    require(ready.status() == dui::SurfaceAcquireStatus::consumed, "taken acquisition remained ready");
    ...
    require(
        dui::SurfaceRequest{{100.0, 50.0}, 200, 100, 2.0, 9}.valid(),
        "valid surface request was rejected"
    );
    require(
        !dui::SurfaceRequest{{100.0, 50.0}, 0, 100, 2.0, 9}.valid(),
...
protected:
    [[nodiscard]] dui::SurfaceAcquisition do_acquire(
        const dui::SurfaceRequest& request
    ) override {
        ...
        return dui::SurfaceAcquisition::ready(std::make_unique<dui::SurfaceFrame>(
            ...
            std::make_unique<MemoryFrameDelegate>(
                state,
                static_cast<std::size_t>(width) * request.physical_height * 4
            )
        ));
    }
};
...
void raster_surface_validates_request_and_acquired_frame() {
    ...
    require(acquisition.status() == dui::SurfaceAcquireStatus::ready, "valid surface was not acquired");
...
public:
    ...
    void render(
        dui::SurfaceFrame& frame,
        const dui::LayerTree& layer_tree,
        const dui::SurfaceRequest& request
    ) override {
...
void raster_thread_submits_immutable_frames_in_order() {
    ...
    dui::RasterThread raster{surface, [log] {
        return std::make_unique<RecordingSurfaceRenderer>(log);
    }};
    ...
    require(first_ticket.id() != 0 && second_ticket.id() > first_ticket.id(), "raster frame IDs are not monotonic");
    require(first_ticket.wait() == dui::RasterThread::FrameOutcome::presented, "first frame was not presented");
    require(first_ticket.wait() == dui::RasterThread::FrameOutcome::presented, "frame ticket was consumptive");
    require(second_ticket.wait() == dui::RasterThread::FrameOutcome::presented, "second frame was not presented");
    raster.wait_idle();
    ...
    require(log->generations == std::vector<std::uint64_t>{1, 2}, "captured surface generations changed");
    require(surface->state->presents == 2 && surface->state->abandons == 0, "rendered frames were not presented exactly once");
...
void renderer_failure_is_reported_to_waiter() {
    ...
    const auto frame = raster.submit(
        owner.layer_frame(dui::BoxConstraints::tight({100.0, 100.0})),
...
void renderer_callback_can_request_quiescent_stop() {
    ...
    static_cast<void>(raster.submit(
        owner.layer_frame(dui::BoxConstraints::tight({100.0, 100.0})),
        {{100.0, 100.0}, 100, 100, 1.0, 1}
    ));
    raster.wait_idle();
...
void raster_thread_classifies_surface_acquisition_results() {
    ...
    for (const auto status : {
        dui::SurfaceAcquireStatus::unavailable,
        dui::SurfaceAcquireStatus::out_of_date
    }) {
        auto surface = std::make_shared<ContractSurface>();
    ...
    auto log = std::make_shared<RenderLog>();
    dui::RasterThread lost{lost_surface, [log] {
        return std::make_unique<RecordingSurfaceRenderer>(log);
    }};
    ...
    bool mismatch_rejected = false;
    try {
        static_cast<void>(validating.submit(
            tree,
            {{50.0, 50.0}, 50, 50, 1.0, 2}
        ));
    } catch (const std::invalid_argument&) {
        mismatch_rejected = true;
    }
