...
void gesture_arena_selects_one_winner() {
  ...
  require(events == std::vector<std::string>{"drag:accept:1", "tap:reject:1"},
...
void gesture_arena_promotes_last_member_and_cancels() {
  ...
  require(events == std::vector<std::string>{"first:reject:2", "second:accept:2"},
  ...
  arena.add(3, second);
  arena.cancel(3);
  require(events == std::vector<std::string>{"first:reject:3", "second:reject:3"},
...
void eager_accept_waits_for_arena_close() {
  ...
  arena.add(4, late);
  arena.close(4);
  require(events == std::vector<std::string>{"eager:accept:4", "late:reject:4"},
...
public:
  ReentrantArenaMember(dui::GestureArena& arena, bool& rejected)
    : arena_(&arena), rejected_(&rejected) {}
...
void focus_routes_keys_from_leaf_to_ancestor() {
  ...
  require(events == std::vector<std::string>{"child", "parent"},
          "key event followed the wrong focus path");
}
...
void focus_handler_can_replace_itself_safely() {
  ...
  require(manager.dispatch_key({"A", dui::KeyPhase::down}) == dui::KeyEventResult::ignored,
          "first self-replacing key handler returned the wrong result");
  require(manager.dispatch_key({"A", dui::KeyPhase::down}) == dui::KeyEventResult::handled,
...
void focus_reparents_around_destroyed_intermediate_node() {
  ...
  leaf.reset();
  require(manager.focused_node() == root,
          "focus did not fall back through removed intermediate node");
}
...
void text_editing_ranges_use_utf8_boundaries() {
  ...
  require(!dui::TextRange{0, 1}.valid_for("\x80"),
          "range validation accepted malformed UTF-8 text");
}

class InputClient final : public dui::TextInputClient {
public:
  void update_editing_value(dui::TextEditingValue value) override { last_value = std::move(value); }
...
public:
  [[nodiscard]] dui::TextInputSessionId
  start_text_input(std::weak_ptr<dui::TextInputClient> input_client, dui::TextInputConfiguration,
                   dui::TextEditingValue) override {
...
void text_input_backend_holds_weak_client_session() {
  ...
  const auto session = backend.start_text_input(client, {}, {"", {}, std::nullopt});
