...
void animation_controller_runs_forward_and_reverse() {
    ...
    dui::AnimationController animation{
        scheduler,
        dui::FrameDuration{1.0},
    ...
    require(animation.status() == dui::AnimationStatus::completed, "animation status is not completed");
    ...
    require(animation.status() == dui::AnimationStatus::dismissed, "animation status is not dismissed");
...
struct TaskResourceComponent {
    ...
    auto build(dui::BuildContext& context) const {
        ...
        auto& task = context.resource<"task">([&] {
            return delayed_value(*executor, token, *side_effect);
        });
        task.start();
...
dui::Task<void> replace_owning_element(
    dui::ManualExecutor& executor,
    dui::BuildOwner& owner,
    int& side_effect
) {
...
void running_coroutine_survives_destruction_of_its_task_owner() {
    ...
    require(owner.dump_tree().find("replacement") != std::string::npos, "coroutine did not replace its owner");
}

dui::Task<void> destroy_executor_while_running(
    dui::ManualExecutor& executor,
...
void run_all_survives_executor_destruction_from_coroutine() {
    ...
    executor->run_all();
    require(task.done() && side_effect == 1, "run_all accessed an executor destroyed by resumed work");
}
