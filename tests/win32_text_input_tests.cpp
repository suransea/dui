...
class TestWindow {
public:
    TestWindow() : handle_(CreateWindowExW(
        0,
        L"STATIC",
        L"DUI text input test",
        0,
        0,
        0,
        100,
...
void character_editing_and_actions() {
    ...
    require(client->last_value.has_value() && client->last_value->text == "AB", "character was not inserted");
    ...
    require(client->last_action == dui::TextInputAction::done, "Enter did not perform the configured action");
...
void surrogate_pair_and_stale_sessions() {
    ...
    require(
        client->last_value.has_value() && client->last_value->text == "\xf0\x9f\x98\x80",
